...
export default function HabitDetailPage() {
  ...
  const [lastHabitLogId, setLastHabitLogId] = useState<string | null>(null);
  ...
  const handleCompleteToday = async () => {
    ...
    if (result.success) {
      ...
      // Get the habit log ID from the result or fetch it
      if (result.habitLogId) {
        setLastHabitLogId(result.habitLogId);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950">
      ...
      {lastHabitLogId && (
        <ImageUploadModal
          habitLogId={lastHabitLogId}
          userId={user?.id || ''}
          habitName={habit?.name || 'Your Habit'}
          isOpen={showImageModal}
          onClose={() => setShowImageModal(false)}
          onSuccess={() => {
            // Optional: refresh community feed
          }}
        />
      )}
      {/* Top Bar */}
